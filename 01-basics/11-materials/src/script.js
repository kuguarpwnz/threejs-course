...
const doorAmbientOcclsionTexture = textureLoader.load('/textures/door/ambientOcclusion.jpg')
...
material.map = doorColorTexture // setting later example
material.aoMap = doorAmbientOcclsionTexture
...
gui.add(material.iridescenceThicknessRange, '0').min(1).max(1000).step(1).name('iridescence from')
gui.add(material.iridescenceThicknessRange, '1').min(1).max(1000).step(1).name('iridescence to')
...
const geometry3 = new THREE.TorusGeometry(0.3, 0.2, 64, 128)
